...
defineOptions({ name: 'XVListTable' });
const props = defineProps<Props>();
const elRef = ref<HTMLDivElement>();
const eventIds = shallowRef<
  Partial<Record<keyof Events, number | EventHandlerVCHART>>
>({});
const listTable = shallowRef<Instance>();
watch(
  () => props.options,
  (options) => {
    options && listTable.value?.updateOption(options);
  },
...
watch(
  ...
  (records) => {
    records && listTable.value?.setRecords(records);
  },
);
watch(
  () => props.events,
...
const updateEvents = (events: Events) => {
  ...
  Object.keys(eventIds.value).forEach((key) => {
    ...
    typeof eventId === 'number'
      ? listTable.value?.off(eventId || 0)
      : listTable.value?.offVChartEvent(
          key.replace(/VCHART-/, ''),
    ...
    if (key.startsWith('VCHART-')) {
      (<T extends EventTypeVCHART>(type: T, handler: EventHandlerVCHART) => {
        ...
        if (typeof handler === 'function') {
          ...
          listTable.value?.onVChartEvent(_type, handler);
        } else if (handler) {
          const { query, handler: _handler } = handler;
          if (_handler) {
            query
              ? listTable.value?.onVChartEvent(_type, query, _handler)
              : listTable.value?.onVChartEvent(_type, _handler);
          }
        }
      ...
      (<T extends EventType>(type: T, handler: EventHandler<T>) => {
        if (handler) {
          eventIds.value[type] = listTable.value?.on(
            ListTable.EVENT_TYPE[type],
            handler,
          );
        }
...
onMounted(() => {
  if (elRef.value) {
    listTable.value = new ListTable(elRef.value, {
      ...(props.options || {}),
...
onUnmounted(() => {
  ...
  listTable.value?.release();
});
defineExpose<Expose>({
  execMethod(method, args) {
    return listTable.value?.[method]?.(...args);
  },
  readAttribute(attr) {
    return listTable.value?.[attr];
  },
  getInstance() {
