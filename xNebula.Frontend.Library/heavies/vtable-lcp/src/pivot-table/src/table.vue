...
import {
  ...
  type PivotTableEventType,
  type PivotTableEventHandler,
  type PivotTableEventTypeVCHART,
  type PivotTableEventHandlerVCHART,
} from '@xnebula/vtable';
import type { Method, Attribute } from '@xnebula/utils';

type EventHandler<T> = T extends PivotTableEventType
  ? PivotTableEventHandler<T>
  : T extends PivotTableEventTypeVCHART
    ? PivotTableEventHandlerVCHART
    : never;
...
// 计算配置部分
const options = computed<PivotTableOptions>(() =>
...
// 事件转化
const fnEvents = shallowRef<PivotTableEvents>({});
Object.keys(events).forEach((_key) => {
  (<T extends keyof PivotTableEvents>(key: T) => {
    ...
    }) as PivotTableEvents[T];
  })(_key as keyof PivotTableEvents);
});
// 对外暴露方法
const tableRef = ref<InstanceType<typeof XVPivotTable>>();
defineExpose<
  {
    ...
    execMethod: (
      params: Omit<MethodParamsType, 'args'> & {
        args: {
          methodName: Parameters<Method<PivotTableInstance>>[0];
          methodArgs: Parameters<Method<PivotTableInstance>>[1];
        };
      },
    ) => ReturnType<Method<PivotTableInstance>>;
    readAttribute: (
      params: Omit<MethodParamsType, 'args'> & {
        args: { attrName: Parameters<Attribute<PivotTableInstance>>[0] };
      },
    ) => ReturnType<Attribute<PivotTableInstance>>;
  } & RegisterMethodsType
>({
  ...methods,
  reload() {
