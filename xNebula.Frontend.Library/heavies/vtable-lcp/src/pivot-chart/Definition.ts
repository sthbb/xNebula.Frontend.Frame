...
import { PivotChartEventTypes, type PivotChartOptions } from '@xnebula/vtable';

type Events = Partial<
  Record<keyof typeof PivotChartEventTypes, ActionsObjectSchemaType>
>;

export const Definition: IWidgetDefinition = {
  name: 'XVPivotChart',
  label: 'XVPivotChart',
  description: 'XVPivotChart 表格组件',
  ...
  propsDefinition: {
    ...
    options: {
      type: [Object, String] as PropType<PivotChartOptions | string>,
      required: false,
      default: () => {},
  ...
  eventsDefinition: {
    // PivotChartEventTypes 这个事件列表里不包含 VChart 事件
    // 即不包含以 VCHART- 开头的事件，因为暂时搜集不到，要手动搜集
    // 最好事件能够提供输入方式，让用户去输入
    supports: [
      ...baseDefinitions.baseWidgetDefinition.methodsDefinition.supports,
      // 设计器是否可以提供输入事件名称，然后配置该事件的 actions
      // 渲染器 registerCommon 内部组装事件 actions 应该遍历 props.events 的 key，而不是遍历这个列表
      ...Object.keys(PivotChartEventTypes).map((key) => ({
        name: key,
        label: key,
        description: `${key}事件`,
      })),
