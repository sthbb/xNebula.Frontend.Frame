...
import { ListTableEventTypes, type ListTableOptions } from '@xnebula/vtable';

type Events = Partial<
  Record<keyof typeof ListTableEventTypes, ActionsObjectSchemaType>
>;

export const Definition: IWidgetDefinition = {
  name: 'XVListTable',
  label: 'XVListTable',
  description: 'XVListTable 表格组件',
  ...
  propsDefinition: {
    ...
    options: {
      type: [Object, String] as PropType<ListTableOptions | string>,
      required: false,
      default: () => {},
  ...
  eventsDefinition: {
    supports: [
      ...
      ...Object.keys(ListTableEventTypes).map((key) => ({
        name: key,
        label: key,
        description: `${key}事件`,
      })),
