# @xnebula/minio-upload Minio上传组件
...
<template>
  ...
  <UploadFile ref="uploadFile" :minio-data="minioData" :minio-api="minioApi" @uploading="uploadingHandle"  @success="successHandle" @pause="pauseHandle" @resume="resumeHandle" @delete="deleteHandle" @download="downloadHandle" @error="errorHandle" />
  <!-- 单个图片上传 -->
  <UploadImage :minio-data="minioData" :minio-api="minioApi" :minio-src="minioSrc" @success="successHandle" @pause="pauseHandle" @resume="resumeHandle" @delete="deleteHandle" @download="downloadHandle" @error="errorHandle" />
  <!-- 多个文件上传 -->
  <el-button type="success" @click="dialogToggleHandle">
    <el-icon color="#fff" size="16px"><Upload /></el-icon>
    MinIO上传
  </el-button>
  <UploadMultipleFile ref="uploadMultipleFile" :minio-data="minioData" :minio-api="minioApi" @beforeClose="beforeCloseHandle" @error="errorHandle" />
</template>

<script setup>
import { ref } from "vue";
import { Upload } from "@element-plus/icons-vue";
import "@xnebula/minio-upload/index.css";
import { UploadFile, UploadImage, UploadMultipleFile, setAxiosInstance, setAxios } from "@xnebula/minio-upload";

// 当前版本与@xnebula/frame结合使用需要实现setAxios与setAxiosInstance方法，代码如下。
import { XCommons } from '@xnebula/frame';
const { request, axios } = XCommons;
